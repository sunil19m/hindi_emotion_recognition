...
def find_lyrics_score_quadrant(lyrics_score):
	...
	for key, value in lyrics_score.items():
		...
		elif (value[1] >= value[0]) and  (value[1] >= value[2]) and (value[1] >= value[3]):
			lyrics_score_axis[key] = 2
		elif (value[2] >= value[0]) and  (value[2] >= value[1]) and (value[2] >= value[3]):
			lyrics_score_axis[key] = 3
		elif (value[3] >= value[0]) and  (value[3] >= value[1]) and (value[3] >= value[2]):
...
def lyrics_score(lyrics_sentence_score):
	...
	for key, value in lyrics_sentence_score.items():
		if value:
			...
			qtr3 = 0
			qtr4 = 0
			for i, val in enumerate(range(len(value[VALENCE_MEAN]))):
				filter = 0
				if (value[VALENCE_MEAN][i] < filter and value[VALENCE_MEAN][i] > (-1 * filter)) \
					or (value[AROUSAL_MEAN][i] < filter and value[AROUSAL_MEAN][i] > (-1 * filter)):
					continue
				if value[VALENCE_MEAN][i] >= 0 and value[AROUSAL_MEAN][i] >= 0:
					qtr1 = qtr1 + 1
				if value[VALENCE_MEAN][i] < 0 and value[AROUSAL_MEAN][i] > 0:
					qtr2 = qtr2 + 1
				if value[VALENCE_MEAN][i] < 0 and value[AROUSAL_MEAN][i] < 0:
				...
				if value[VALENCE_MEAN][i] >= 0 and value[AROUSAL_MEAN][i] <= 0:
					qtr4 = qtr4 + 1
			lyrics_score[key] = (qtr1, qtr2, qtr3, qtr4)
	return lyrics_score
...
def main():
	...
	"""
	json_data = dict()
	json_data["3"] = json_lyrics_data["3"]
