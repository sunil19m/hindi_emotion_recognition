...
def find_max_valence_arousal(V1, A1, V2, A2 ):
	...
	max_valence_v1 = np.sum(V1)/(len(V1) * 1.0)
	max_arousal_a1 = np.sum(A1)/(len(A1) * 1.0)
	max_valence_v2 = np.sum(V2)/(len(V2) * 1.0)
	max_arousal_a2 = np.sum(A2)/(len(A2) * 1.0)

	weighted_avg_valence = (((max_valence_v1 * len(V1)) + (max_valence_v2 * len(V2))))/2.0
	weighted_avg_arousal = (((max_arousal_a1 * len(A1)) + (max_arousal_a2 * len(A2))))/2.0
	return (weighted_avg_valence, weighted_avg_arousal)
