...
def find_sentence_valence_arousal(sentence_emotion_dict):
	...
	for key, value in sentence_emotion_dict.items():
		valence_mean.append(value[VALENCE_MEAN_INDEX])
		arousal_mean.append(value[AROUSAL_MEAN_INDEX])
...
def get_lyrical_score(anhw_model, adverb_model, json_lyrics_data):
	...
	for id, value in json_lyrics_data.items():
		...
		for sentence in lyrics_split:
			...
			sentence_emotion_dict.update(find_sentence_emotion_score(stemmed_sentence, anhw_model, adverb_model))
		...
		if sentence_emotion_dict:
			...
			lyrics_emotion[id][VALENCE_MEAN].extend(sentence_valence)
			lyrics_emotion[id][AROUSAL_MEAN].extend(sentence_arousal)
			#print ("===========================================")
		#print ("*********************************************************************")
...
def main():
	...
	"""
	json_data = dict()
	json_data["14"] = json_lyrics_data["14"]
